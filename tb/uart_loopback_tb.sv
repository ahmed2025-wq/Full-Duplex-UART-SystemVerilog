...
module uart_grading_tb;
  ...
  localparam int unsigned count_width = 3;
  ...
  //-------------------------------------------------------------------------
  UART_TX #(.Data_Width(DATA_W)  , .Depth(Depth) ) u_tx (
    .clk     (clk),
    .rst  (rst_n),
    .tx_data_in   (tx_data),
    .wr_en   (tx_valid),
    .parity_en  (par_en),
    .parity_bit_state (par_odd),
    .Busy    (tx_busy),
    .TX_OUT      (line),
  ...
  UART_RX #(.DATA_WIDTH(DATA_W)  ) u_rx (
    .i_clk        (clk),
    .i_rst        (rst_n),
    .i_rx_in         (line),
    .i_parity_en     (par_en),
    .i_Parity_type    (par_odd),
    .o_rx_out       (rx_data),
    .o_rx_valid      (rx_valid),
    .o_Busy       (rx_busy),
    .o_parity_error (parity_err),
    .o_fram_error  (frame_err)
  );
